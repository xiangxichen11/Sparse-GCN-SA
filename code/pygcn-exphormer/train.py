...
from utils import accuracy, randomsplit, acquiretvt, ratio_homo, pca
...
parser.add_argument('--trainsplit', type=float, default=0.48, help='trainsplit.')
parser.add_argument('--K', type=int, default=3, help='Feature aggregation times.')
...
parser.add_argument('--hd', type=int, default=48, help='Number of hidden units.')
parser.add_argument('--dropout', type=float, default=0.5, help='Dropout rate (1 - keep probability).')
parser.add_argument('--epsilon', type=float, default=0.9, help='threshold value of similarity')
...
# optimal_components = max_delta_index + 1
...
testacc = np.zeros(10)
for j in range(10):
    idx_train, idx_val, idx_test = acquiretvt(args.dataset, args.trainsplit, labels, j)
    model = GCNSA(nfeat=features.shape[1],
                nhid=args.hd,
    ...
    def train(epoch):
        ...
        loss_train = F.nll_loss(output[idx_train], labels[idx_train]) #torch.nn.functional.cross_entropy
