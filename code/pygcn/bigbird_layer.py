...
class BigBirdBlockSparseAttention(nn.Module):
  ...
  def __init__(self, config, seed=None):
    ...
    self.query = nn.Linear(config.dim_hidden,
                           self.all_head_size,
                           bias=config.use_bias)
    self.key = nn.Linear(config.dim_hidden,
                         self.all_head_size,
                         bias=config.use_bias)
    self.value = nn.Linear(config.dim_hidden,
                           self.all_head_size,
                           bias=config.use_bias)
  ...
  def forward(
    ...
    context_layer = context_layer.contiguous().view(batch_size, from_seq_length,
                                                    -1)
  ...
  def torch_bmm_nd(inp_1, inp_2, ndim=None):
    ...
    return torch.bmm(
        ...
        inp_2.reshape((-1,) + inp_2.shape[-2:])).view(inp_1.shape[:ndim - 2] +
                                                      (inp_1.shape[ndim - 2],
                                                       inp_2.shape[ndim - 1]))
  ...
  def torch_bmm_nd_transpose(inp_1, inp_2, ndim=None):
    ...
    return torch.bmm(
        inp_1.reshape((-1,) + inp_1.shape[-2:]),
        inp_2.reshape((-1,) + inp_2.shape[-2:]).transpose(
            1, 2)).view(inp_1.shape[:ndim - 2] +
                        (inp_1.shape[ndim - 2], inp_2.shape[ndim - 2]))
  ...
  def bigbird_block_sparse_attention(
    ...
    if from_seq_len in [1024, 3072, 4096]:  # old plans used in paper
      rand_attn = [
          self._bigbird_block_rand_mask(
              ...
              to_block_size,
              n_rand_blocks,
              last_idx=1024)[:(from_seq_len // from_block_size - 2)]
    ...
    rand_attn = torch.tensor(rand_attn,
                             device=query_layer.device,
                             dtype=torch.long)
    rand_attn.unsqueeze_(0)
    rand_attn = torch.cat([rand_attn for _ in range(batch_size)], dim=0)

    rand_mask = self._create_rand_mask_from_inputs(from_blocked_mask,
                                                   to_blocked_mask, rand_attn,
                                                   n_heads, n_rand_blocks, bsz,
                                                   from_seq_len,
                                                   from_block_size)

    blocked_query_matrix = query_layer.view(bsz, n_heads,
                                            from_seq_len // from_block_size,
                                            from_block_size, -1)
    blocked_key_matrix = key_layer.view(bsz, n_heads,
                                        to_seq_len // to_block_size,
                                        to_block_size, -1)
    blocked_value_matrix = value_layer.view(bsz, n_heads,
                                            to_seq_len // to_block_size,
                                            to_block_size, -1)
    ...
    first_product = self.torch_bmm_nd_transpose(blocked_query_matrix[:, :, 0],
                                                key_layer,
                                                ndim=4)
    ...
    first_attn_weights = nn.functional.softmax(
        first_product, dim=-1)  # [bsz, n_heads, from_block_size, to_seq_len]

    # [bsz, n_heads, from_block_size, to_seq_len] x [bsz, n_heads, to_seq_len, -1] ==> [bsz, n_heads, from_block_size, -1]
    first_context_layer = self.torch_bmm_nd(first_attn_weights,
                                            value_layer,
                                            ndim=4)
    ...
    second_product = self.torch_bmm_nd_transpose(blocked_query_matrix[:, :, 1],
                                                 second_key_mat,
                                                 ndim=4)
    ...
    second_product += (1.0 - torch.minimum(second_seq_pad,
    ...
    second_context_layer = self.torch_bmm_nd(second_attn_weights,
                                             second_value_mat,
                                             ndim=4)
    ...
    inner_band_product = self.torch_bmm_nd_transpose(middle_query_matrix,
    ...
    first_band_product += (
        1.0 - to_mask[:, :, :, :to_block_size].unsqueeze(3)) * attn_mask_penalty
    last_band_product += (1.0 - to_mask[:, :, :, -to_block_size:].unsqueeze(3)
                         ) * attn_mask_penalty
    ...
    context_layer = self.torch_bmm_nd(attn_weights[:, :, :, :, to_block_size:4 *
                                                   to_block_size],
    ...
    # [bsz, n_heads, from_block_size, -1] x [bsz, n_heads, (4+n_rand_blocks)*to_block_size, -1] ==> [bsz, n_heads, from_block_size, (4+n_rand_blocks)*to_block_size]
    second_last_product = self.torch_bmm_nd_transpose(blocked_query_matrix[:, :,
                                                                           -2],
                                                      second_last_key_mat,
                                                      ndim=4)
    ...
    second_last_product += (1.0 - torch.minimum(
        second_last_seq_pad, second_last_rand_pad)) * attn_mask_penalty
    ...
    # [bsz, n_heads, from_block_size, -1] x [bsz, n_heads, to_seq_len, -1] ==> [bsz, n_heads, from_block_size, to_seq_len]
    last_product = self.torch_bmm_nd_transpose(blocked_query_matrix[:, :, -1],
                                               key_layer,
                                               ndim=4)
    last_product = last_product * rsqrt_d
    last_product += (1.0 - to_mask) * attn_mask_penalty
    last_attn_weights = nn.functional.softmax(
        last_product, dim=-1)  # [bsz, n_heads, from_block_size, n]
    ...
    if output_attentions:
      ...
      attention_probs[:, :, :
                      from_block_size, :] = first_attn_weights  # all keys global

      # 2nd query block
      # corresponding to `second_context_layer`
      attention_probs[:, :, from_block_size:2 * from_block_size, :3 *
                      to_block_size] = second_attn_weights[:, :, :, :3 *
                                                           to_block_size]  # 1st three key blocks (global + sliding)
      attention_probs[:, :, from_block_size:2 * from_block_size,
                      -to_block_size:] = second_attn_weights[:, :, :, 3 *
                                                             to_block_size:4 *
                                                             to_block_size]  # last key block (global)
      # random keys
      for p1, i1, w1 in zip(range(bsz), rand_attn, second_attn_weights):
        ...
        for p2, i2, w2 in zip(range(n_heads), i1, w1):
          ...
          attn_probs_view[p1, p2, 1, :,
                          i2[0]] = right_slice.view(from_block_size,
                                                    n_rand_blocks,
                                                    to_block_size)
      ...
      for q_idx in range(from_seq_len // from_block_size - 4):
        ...
        right_slice = attn_weights[:, :, q_idx, :,
                                   to_block_size:4 * to_block_size]
        attn_probs_view[:, :, q_idx, :, q_idx:q_idx + 3, :] = right_slice.view(
            bsz, n_heads, from_block_size, 3,
            to_block_size)  # inner_band_product
      # global keys (corresponding to 1st key block)
      attention_probs[:, :, 2 * from_block_size:-2 * from_block_size, :
                      to_block_size] = attn_weights[:, :, :, :, :
                                                    to_block_size].view(
                                                        bsz, n_heads, -1,
                                                        to_block_size
                                                    )  # first_band_product
      ...
      attention_probs[:, :, -2 * from_block_size:-from_block_size, :
                      to_block_size] = second_last_attn_weights[:, :, :, :
                                                                to_block_size]  # 1st key block (global)
      attention_probs[:, :, -2 * from_block_size:-from_block_size, -3 *
                      to_block_size:] = second_last_attn_weights[:, :, :,
                                                                 to_block_size:
                                                                 4 *
                                                                 to_block_size]  # last three blocks (global + sliding)
      # random keys
      for p1, i1, w1 in zip(range(bsz), rand_attn, second_last_attn_weights):
        ...
        for p2, i2, w2 in zip(range(n_heads), i1, w1):
          ...
          right_slice = w2[:, 4 * to_block_size:]
          attn_probs_view[p1, p2, -2, :,
                          i2[-1]] = right_slice.view(from_block_size,
                                                     n_rand_blocks,
                                                     to_block_size)
      ...
      attention_probs[:, :,
                      -from_block_size:, :] = last_attn_weights  # all keys global
  ...
  def torch_gather_b2(params, indices):
    ...
    # )
    indices_shift = torch.div(torch.arange(
        indices.shape[0] * indices.shape[1] * num_indices_to_gather,
        device=indices.device),
                              num_indices_to_gather,
                              rounding_mode='trunc') * num_indices_to_pick_from
    ...
    out = out_flattened.reshape(params.shape[:2] + (num_indices_to_gather,) +
                                params.shape[3:])
    return out
  ...
  @staticmethod
  def _bigbird_block_rand_mask(from_seq_length,
                               to_seq_length,
                               from_block_size,
                               to_block_size,
                               num_rand_blocks,
    ...
    assert (from_seq_length // from_block_size == to_seq_length //
            to_block_size), "Error the number of blocks needs to be same!"

    rand_attn = np.zeros(
        (from_seq_length // from_block_size - 2, num_rand_blocks),
        dtype=np.int32)
    middle_seq = np.arange(1,
                           to_seq_length // to_block_size - 1,
                           dtype=np.int32)
      ...
      # Missing -4: should have been sliced till last-4
        ...
        if start > last:
          ...
          rand_attn[i - 1, :] = np.random.permutation(
              np.concatenate(
                  (middle_seq[:start], middle_seq[end + 1:last])))[:r]
    return rand_attn
  ...
  def _get_single_block_row_attention(
    ...
    to_block_list = np.arange(to_start_block_id,
                              to_end_block_id,
                              dtype=np.int32)
    # permute the blocks
...
class BigBirdAttention(nn.Module):
  ...
  def forward(
    ...
    if self.attention_type == "original_full":
      ...
      self_outputs = self.self(hidden_states, band_mask, from_mask, to_mask,
...
class BigBirdIntermediate(nn.Module):
  ...
  def __init__(self, config):
    super().__init__()
    self.dense = nn.Linear(config.dim_hidden,
                           config.dim_hidden)  # dim_hidden -> intermediate_size
...
class BigBirdOutput(nn.Module):
  ...
  def __init__(self, config):
    super().__init__()
    self.dense = nn.Linear(config.dim_hidden,
                           config.dim_hidden)  # intermediate -> hidden
    self.LayerNorm = nn.LayerNorm(config.dim_hidden, eps=config.layer_norm_eps)
    self.dropout = nn.Dropout(config.dropout)
...
class BigBirdModel(nn.Module):
  ...
  def forward(self, batch, **kwargs):
    ...
    ) * self.config.block_size  # returned +5 in parentheses
  ...
  def create_masks_for_block_sparse_attn(attention_mask: torch.Tensor,
    ...
    blocked_encoder_mask = attention_mask.view(batch_size,
  ...
  def _pad_to_block_size(
    ...
    if padding_len > 0:
      ...
      inputs_embeds_padding = torch.zeros((batch_size, padding_len, emb_dim),
                                          dtype=torch.float,
