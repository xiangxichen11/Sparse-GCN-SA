...
def load_data(dataset_name, train_percentage=None):
  ...
  if dataset_name == 'film':
    with open(graph_node_features_and_labels_file_path
      ...
      for line in graph_node_features_and_labels_file:
        ...
        assert (len(line) == 3)
        assert (int(line[0]) not in graph_node_features_dict and
                int(line[0]) not in graph_labels_dict)
    ...
    with open(graph_node_features_and_labels_file_path
      ...
      for line in graph_node_features_and_labels_file:
        ...
        graph_node_features_dict[int(line[0])] = np.array(
            line[1].split(','), dtype=np.uint8)  # 按照','把特征字符串进行分割
        graph_labels_dict[int(line[0])] = int(line[2])
  ...
  ])  # key=lambda x: x是固定用法，0代表按第一个元素排序，
  labels = np.array(
      [label for _, label in sorted(G.nodes(data='label'), key=lambda x: x[0])])

  assert (train_percentage is not None and val_percentage is not None)
  assert (train_percentage < 1.0 and val_percentage < 1.0 and
          train_percentage + val_percentage < 1.0)

  labels = th.tensor(labels, dtype=th.int8)
  [idx_train, idx_val, idx_test] = randomsplit(labels, train_percentage,
                                               val_percentage)
