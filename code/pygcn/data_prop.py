...
def load_citation(dataset_str="cora",
  ...
  for i in range(len(names)):
    with open(
        os.path.join(data_path, "ind.{}.{}".format(dataset_str.lower(),
                                                   names[i])), 'rb') as f:
  ...
  if task_type == "full":
    ...
    idx_val = range(len(ally) - 500, len(ally))  #验证集是500
  elif task_type == "semi":
...
def loadRedditFromNPZ(dataset_dir=datadir):
  ...
  return adj, data['feats'], data['y_train'], data['y_val'], data[
      'y_test'], data['train_index'], data['val_index'], data['test_index']
...
def data_loader(dataset,
  ...
  if dataset == "reddit":
    ...
    (adj, features, labels, idx_train, idx_val, idx_test, degree,
     learning_type) = load_citation(dataset, normalization, porting_to_torch,
                                    data_path, task_type)
    train_adj = adj
