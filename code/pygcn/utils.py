...
def acquiretvt(dataset, trainsplit, labels, i):
  if dataset == 'cora':
    if trainsplit == 0.48:
      tvt_index = np.load('./tvtsplit/cora/0.48/' + 'cora_tvt_index' + str(i) +
                          '.npy')
    else:
      tvt_index = np.load('./tvtsplit/cora/0.6/' + 'cora_tvt_index' + str(i) +
                          '.npy')
  if dataset == 'citeseer':
    if trainsplit == 0.48:
      tvt_index = np.load('./tvtsplit/citeseer/0.48/' + 'citeseer_tvt_index' +
                          str(i) + '.npy')
    else:
      tvt_index = np.load('./tvtsplit/citeseer/0.6/' + 'citeseer_tvt_index' +
                          str(i) + '.npy')
  if dataset == 'pubmed':
  ...
  if dataset == 'squirrel':
    if trainsplit == 0.48:
      tvt_index = np.load('./tvtsplit/squirrel/0.48/' + 'squirrel_tvt_index' +
                          str(i) + '.npy')
    else:
      tvt_index = np.load('./tvtsplit/squirrel/0.6/' + 'squirrel_tvt_index' +
                          str(i) + '.npy')
  if dataset == 'chameleon':
    if trainsplit == 0.48:
      tvt_index = np.load('./tvtsplit/chameleon/0.48/' + 'chameleon_tvt_index' +
                          str(i) + '.npy')
    else:
      tvt_index = np.load('./tvtsplit/chameleon/0.6/' + 'chameleon_tvt_index' +
                          str(i) + '.npy')
  if dataset == 'cornell':
    if trainsplit == 0.48:
      tvt_index = np.load('./tvtsplit/cornell/0.48/' + 'cornell_tvt_index' +
                          str(i) + '.npy')
    else:
      tvt_index = np.load('./tvtsplit/cornell/0.6/' + 'cornell_tvt_index' +
                          str(i) + '.npy')
  if dataset == 'texas':
    if trainsplit == 0.48:
      tvt_index = np.load('./tvtsplit/texas/0.48/' + 'texas_tvt_index' +
                          str(i) + '.npy')
    else:
      tvt_index = np.load('./tvtsplit/texas/0.6/' + 'texas_tvt_index' + str(i) +
                          '.npy')
  if dataset == 'wisconsin':
    if trainsplit == 0.48:
      tvt_index = np.load('./tvtsplit/wisconsin/0.48/' + 'wisconsin_tvt_index' +
                          str(i) + '.npy')
    else:
      tvt_index = np.load('./tvtsplit/wisconsin/0.6/' + 'wisconsin_tvt_index' +
                          str(i) + '.npy')
  tvt_index = th.tensor(tvt_index)
  train_n = int(np.ceil(tvt_index.size(0) * trainsplit + th.max(labels)))
  val_n = int(
      np.ceil(tvt_index.size(0) * (1 - trainsplit - 0.2) + th.max(labels)))
