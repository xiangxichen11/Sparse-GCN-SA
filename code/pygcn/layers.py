...
class adjcentmatrix(Module):
  ...
  def __init__(self, in_features, epsilon, n_samples, nhid, dropout1, r):
    ...
    self.weight1 = Parameter(
        torch.FloatTensor(self.in_features, self.out_features * self.n_head))
  ...
  def forward(self, input):
    attention2 = torch.zeros(self.n_samples, self.n_samples)
    attention2 = attention2.cpu()
    zero_matrix = attention2
    inter_put1 = torch.mm(input, self.weight1)
    inter_put3 = torch.mm(input, self.weight3)
    j = 0
    adjacent = zero_matrix.cuda
    for i in range(self.n_head):
      input1 = inter_put1[:, j:j + self.out_features]
      adjacent = +consinesimilar(input1, input1)
    ...
    attention = attention + torch.eye(self.n_samples).cpu()
...
class MultiHeadAttention(torch.nn.Module):
  ...
  def __init__(self, input_dim, n_heads, ouput_dim=None):  # [1966,4]
    ...
    self.W_Q = torch.nn.Linear(input_dim, self.d_k * self.n_heads,
                               bias=False)  # [1966,491]*4
    self.W_K = torch.nn.Linear(input_dim, self.d_k * self.n_heads,
                               bias=False)  # [1966,491]*4
    self.W_V = torch.nn.Linear(input_dim, self.d_v * self.n_heads,
                               bias=False)  # [1966,491]*4
    self.fc = torch.nn.Linear(self.n_heads * self.d_v,
                              self.ouput_dim,
                              bias=False)  # [1966,491]*4

  def forward(self, X):
    ## (S, D) -proj-> (S, D_new) -split-> (S, H, W) -trans-> (H, S, W)
    Q = self.W_Q(X).view(-1, self.n_heads, self.d_k).transpose(
        0, 1)  # [256,1966]->[256,1964]->[256,4,491]->[4,256,625]
    K = self.W_K(X).view(-1, self.n_heads, self.d_k).transpose(
        0, 1)  # [256,1966]->[256,1964]->[256,4,491]->[4,256,625]
    V = self.W_V(X).view(-1, self.n_heads, self.d_v).transpose(
        0, 1)  # [256,1966]->[256,1964]->[256,4,491]->[4,256,625]

    scores = torch.matmul(Q, K.transpose(-1, -2)) / np.sqrt(
        self.d_k)  # [4,256,491]*[4,491,256]->[4,256,256]
